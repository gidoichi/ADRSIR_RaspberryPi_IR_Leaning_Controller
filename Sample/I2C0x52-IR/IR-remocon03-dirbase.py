...
#
#
# ファイル名：IR-remocon03-dirbase.py　　　python3用
...
#
#  実行方法： ./IR-remocon03-dirbase.py rd testdir
#
...
# ＰＩＣ←ラズハ゜イ　ファイル送信コマンド（t:transfer)：、読込ファイル名（filename)
#
#  ******Ｉ２Ｃ関係内部コマンド
...
# cmd W2_data_num_write 0x29 bus-write(ADR,cmd,n)
...
def read_command(filename, memo_no):
    ...
    if data_num < 65535:
        ...
        block_dat = bus.read_i2c_block_data(
            SLAVE_ADDRESS, R3_data_read, 1
        )  # = 0x35 #bus-read(ADR,cmd,n)
        for _ in range(data_num):
            block_dat = bus.read_i2c_block_data(
                SLAVE_ADDRESS, R3_data_read, 4
            )  # = 0x35 #bus-read(ADR,cmd,n)
...
while True:
    ...
    if argc == 3:
        ...
        if command == "t":
            filename = sys.argv[2]
            trans_command(filename)
            print("file_trans_mode_end")
            break
        ...
        if command == "wd":
            ...
            print("wd_mode_end")
            break
    print("usage:python3 IR-remocon03-commandline.py rd testdir")
    break
